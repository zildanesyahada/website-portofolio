...
function StatusCard({ className = '', onClick }) {
  return (
    <motion.a
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      href="#contact"
      onClick={(e) => onClick && onClick(e, 'contact')}
...
export default function Hero() {
  // Handler scroll presisi agar berhenti tepat di pill section
  ...
  return (
    <section id="home" className="scroll-mt-24 pb-4 pt-4 lg:flex lg:min-h-[calc(100svh-84px)] lg:items-start lg:pb-8 lg:pt-20">
      <Container>
        <div className="relative rounded-[28px] border border-line bg-surface lg:min-h-[520px] 2xl:min-h-[600px]">
          ...
          <div className="relative lg:hidden">
            ...
            <StatusCard 
              ...
              className="absolute inset-x-4 -bottom-8 flex md:inset-x-auto md:left-1/2 md:w-[400px] md:-translate-x-1/2" 
            />
          </div>
          ...
          <StatusCard 
            onClick={handleScroll}
            className="absolute bottom-8 right-8 z-10 hidden w-[300px] lg:flex 2xl:w-[340px]" 
          />
          ...
          <div className="relative z-10 p-6 pt-14 md:p-10 md:pt-16 lg:max-w-[600px] lg:p-16 2xl:max-w-[720px]">
            ...
            >
              I build web apps
              <span className="block font-normal italic text-clay">end to end.</span>
            </Reveal>
            ...
            <Reveal
              delay={0.45}
              className="mt-9 grid grid-cols-3 gap-2 border-t border-line pt-6 sm:gap-6 lg:max-w-[480px]"
            >
              {stats.map((s) => (
                <div key={s.label}>
                  <p className="font-serif text-2xl font-semibold leading-tight sm:text-[28px] lg:text-[32px]">
                    <CountUp to={s.to} suffix={s.suffix} />
                  </p>
                  <p className="mt-0.5 text-[11px] text-muted sm:text-[13px]">{s.label}</p>
                </div>
              ))}
            </Reveal>
          </div>
